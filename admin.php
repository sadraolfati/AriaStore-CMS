...
// قطع دسترسی موتورهای جستجو به پنل مدیریت در سطح هدر HTTP
header("X-Robots-Tag: noindex, nofollow, noarchive, nosnippet", true);

// -------------------------------------------------------------------------
// 1. اتصال به پایگاه داده و توابع نصب و راه‌اندازی (مشترک با index.php)
// -------------------------------------------------------------------------
function get_db() {
...
// -------------------------------------------------------------------------
// 2. اتصال دیتابیس و احراز هویت مدیر
// -------------------------------------------------------------------------
$db = get_db();
...
if (!isset($_SESSION['admin_logged_in']) || $_SESSION['admin_logged_in'] !== true) {
    ...
    <head>
        ...
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <meta name="robots" content="noindex, nofollow, noarchive, nosnippet">
        <title>ورود به مدیریت | اریا استور</title>
        ...
        <style>
            ...
            * { box-sizing: border-box; font-family: 'Peyda', Tahoma, sans-serif; }
            ...
            .login-box { background: #1e293b; border: 1px solid #334155; border-radius: 20px; padding: 36px; width: 100%; max-width: 400px; text-align: center; box-shadow: 0 20px 40px rgba(0,0,0,0.4); color: #fff; }
            .login-box h2 { font-size: 22px; font-weight: 800; color: #818cf8; margin-bottom: 6px; }
            .login-box p { font-size: 13px; color: #94a3b8; margin-bottom: 24px; }
            .form-control { width: 100%; padding: 12px 16px; background: #0f172a; border: 1px solid #334155; border-radius: 10px; color: #fff; font-size: 14px; margin-bottom: 14px; outline: none; transition: 0.2s; }
            .form-control:focus { border-color: #6366f1; }
            .btn-login { width: 100%; padding: 14px; background: #4f46e5; color: #fff; border: none; border-radius: 10px; font-size: 15px; font-weight: 700; cursor: pointer; transition: 0.2s; }
            .btn-login:hover { background: #4338ca; }
            .err { background: #7f1d1d; color: #fecaca; padding: 10px; border-radius: 10px; font-size: 13px; margin-bottom: 16px; }
            .author-badge { margin-top:24px; font-size:12px; color:#94a3b8; }
        </style>
    </head>
    <body>
        <div class="login-box">
            <h2>پنل مدیریت اریا استور</h2>
            <p>مشخصات مدیر را جهت ورود وارد کنید</p>
            ...
            <form method="POST">
                ...
                <button type="submit" name="login" class="btn-login">ورود به پنل</button>
            </form>
            <div class="author-badge">AriaStore CMS v3 Beta 1 | توسط صدرا الفتی</div>
        </div>
    </body>
    </html>
    <?php
...
// -------------------------------------------------------------------------
// 3. عملیات حذف و خروجی (Export JSON و CSV)
// -------------------------------------------------------------------------
if (isset($_GET['action'])) {
    if ($_GET['action'] == 'export_json') {
        $backup = [
            'version' => 'AriaStore CMS v3 Beta 1 by Sadra Olfati',
            ...
            'banners' => $db->query("SELECT * FROM banners")->fetchAll(),
            'menus' => $db->query("SELECT * FROM menus")->fetchAll(),
            'comments' => $db->query("SELECT * FROM comments")->fetchAll()
        ];
        header('Content-Type: application/json; charset=utf-8');
        header('Content-Disposition: attachment; filename="ariastore_backup.json"');
    ...
    if ($_GET['action'] == 'export_csv') {
        ...
        header('Content-Disposition: attachment; filename="ariastore_products.csv"');
        $output = fopen('php://output', 'w');
        fwrite($output, "\xEF\xBB\xBF");
        fputcsv($output, ['ID', 'نام محصول', 'نامک', 'دسته', 'قیمت (ریال)', 'قیمت خط‌خورده (ریال)', 'لینک خرید خارجی', 'بازدید', 'پیشنهاد شگفت‌انگیز']);
        $products = $db->query("SELECT * FROM products ORDER BY id DESC")->fetchAll();
        foreach ($products as $p) {
            fputcsv($output, [
                $p['id'], $p['title'], $p['slug'], $p['category_slug'],
                $p['price'], $p['old_price'], $p['external_link'], $p['views'],
                $p['is_special'] ? 'بله' : 'خیر'
            ]);
        }
        fclose($output);
        exit;
    ...
    if ($_GET['action'] == 'del_product' && isset($_GET['id'])) {
        $db->prepare("DELETE FROM products WHERE id=?")->execute([intval($_GET['id'])]);
        header("Location: admin.php?view=products&msg=" . urlencode("محصول با موفقیت حذف شد."));
        exit;
    }
    if ($_GET['action'] == 'del_category' && isset($_GET['id'])) {
        $db->prepare("DELETE FROM categories WHERE id=?")->execute([intval($_GET['id'])]);
        header("Location: admin.php?view=categories&msg=" . urlencode("دسته‌بندی با موفقیت حذف شد."));
        exit;
    }
    if ($_GET['action'] == 'del_post' && isset($_GET['id'])) {
        $db->prepare("DELETE FROM posts WHERE id=?")->execute([intval($_GET['id'])]);
        header("Location: admin.php?view=posts&msg=" . urlencode("مقاله با موفقیت حذف شد."));
        exit;
    }
    if ($_GET['action'] == 'del_page' && isset($_GET['id'])) {
        $db->prepare("DELETE FROM pages WHERE id=?")->execute([intval($_GET['id'])]);
        header("Location: admin.php?view=pages&msg=" . urlencode("برگه با موفقیت حذف شد."));
        exit;
    }
    if ($_GET['action'] == 'del_slider' && isset($_GET['id'])) {
        $db->prepare("DELETE FROM sliders WHERE id=?")->execute([intval($_GET['id'])]);
        header("Location: admin.php?view=sliders&msg=" . urlencode("اسلاید با موفقیت حذف شد."));
        exit;
    }
    if ($_GET['action'] == 'del_banner' && isset($_GET['id'])) {
        $db->prepare("DELETE FROM banners WHERE id=?")->execute([intval($_GET['id'])]);
        header("Location: admin.php?view=sliders&msg=" . urlencode("بنر با موفقیت حذف شد."));
        exit;
    }
    if ($_GET['action'] == 'del_menu' && isset($_GET['id'])) {
        $db->prepare("DELETE FROM menus WHERE id=?")->execute([intval($_GET['id'])]);
        header("Location: admin.php?view=menus&msg=" . urlencode("آیتم منو با موفقیت حذف شد."));
        exit;
    }
    if ($_GET['action'] == 'del_comment' && isset($_GET['id'])) {
    ...
    if ($_GET['action'] == 'approve_comment' && isset($_GET['id'])) {
        $db->prepare("UPDATE comments SET status='approved' WHERE id=?")->execute([intval($_GET['id'])]);
        header("Location: admin.php?view=comments&msg=" . urlencode("دیدگاه تأیید شد."));
        exit;
    }
}

// -------------------------------------------------------------------------
// 4. پردازش فرم‌های POST (ذخیره در دیتابیس)
// -------------------------------------------------------------------------
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_type'])) {
    ...
    if ($action === 'save_settings') {
        $fields = [
            ...
            'top_bar_text', 'top_bar_phone',
            'show_hero_slider', 'show_categories', 'categories_title',
            'show_middle_banners', 'show_special_offers', 'special_offers_title',
            'show_latest_products', 'latest_products_title',
            'show_blog_slider', 'blog_slider_title',
            'footer_col1_title', 'footer_about',
            'footer_col2_title', 'footer_col2_links',
            'footer_col3_title', 'footer_col3_links',
            'footer_col4_title', 'trust_badges',
            'footer_copyright', 'primary_color', 'secondary_color',
            'custom_social_networks'
        ];
        foreach ($fields as $f) {
            if (isset($_POST[$f])) {
                $db->prepare("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)")->execute([$f, trim($_POST[$f])]);
            }
        }
        header("Location: admin.php?view=settings&msg=" . urlencode("تنظیمات سایت با موفقیت ذخیره شد."));
        exit;
    }

    if ($action === 'save_profile') {
        $u = trim($_POST['admin_user'] ?? 'admin');
        $p = trim($_POST['admin_pass'] ?? '12345');
        $e = trim($_POST['admin_email'] ?? 'admin@ariastore.ir');
        if ($u && $p) {
            $db->prepare("INSERT OR REPLACE INTO admin_users (id, username, password, email) VALUES (1, ?, ?, ?)")->execute([$u, $p, $e]);
        }
        header("Location: admin.php?view=profile&msg=" . urlencode("مشخصات مدیر به‌روزرسانی شد."));
        exit;
    }

    if ($action === 'save_scripts') {
        $fields = ['head_scripts', 'body_scripts', 'footer_scripts', 'trust_badges'];
        foreach ($fields as $f) {
            if (isset($_POST[$f])) {
                $db->prepare("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)")->execute([$f, $_POST[$f]]);
            }
        }
        header("Location: admin.php?view=scripts&msg=" . urlencode("اسکریپت‌ها و نمادها ذخیره شد."));
        exit;
    }
    ...
    if ($action === 'save_product') {
        ...
        $slug = trim($_POST['slug'] ?? '') ?: 'product-' . time();
        $cat = trim($_POST['category_slug'] ?? 'general');
        $price = intval($_POST['price'] ?? 0);
        $old_price = intval($_POST['old_price'] ?? 0);
        $image = trim($_POST['image'] ?? '');
        $external_link = trim($_POST['external_link'] ?? '');
        $description = $_POST['description'] ?? ''; // HTML
        $is_featured = isset($_POST['is_featured']) ? 1 : 0;
        $is_special = isset($_POST['is_special']) ? 1 : 0;

        $gallery_lines = array_filter(array_map('trim', explode("\n", $_POST['gallery'] ?? '')));
        $gallery = json_encode(array_values($gallery_lines), JSON_UNESCAPED_UNICODE);

        $specs = [];
        foreach (explode("\n", $_POST['specs_raw'] ?? '') as $line) {
        ...
        $variables = [];
        $var_json_raw = trim($_POST['variables_json'] ?? '');
        if (!empty($var_json_raw)) {
            $decoded_vars = json_decode($var_json_raw, true);
            if ($decoded_vars && is_array($decoded_vars)) {
        ...
        if ($id) {
            ...
            $stmt->execute([$title, $slug, $cat, $price, $old_price, $image, $gallery, $description, $specs_json, $variables_json, $external_link, $is_featured, $is_special, $id]);
        } else {
            ...
            $stmt->execute([$title, $slug, $cat, $price, $old_price, $image, $gallery, $description, $specs_json, $variables_json, $external_link, $is_featured, $is_special]);
        }
    ...
    if ($action === 'save_category') {
        ...
        $slug = trim($_POST['slug'] ?? '') ?: 'cat-' . time();
        $parent_id = intval($_POST['parent_id'] ?? 0);
        $image = trim($_POST['image'] ?? '');
        $sort_order = intval($_POST['sort_order'] ?? 0);

        if ($id) {
            $db->prepare("UPDATE categories SET name=?, slug=?, parent_id=?, image=?, sort_order=? WHERE id=?")->execute([$name, $slug, $parent_id, $image, $sort_order, $id]);
        } else {
            $db->prepare("INSERT INTO categories (name, slug, parent_id, image, sort_order) VALUES (?, ?, ?, ?, ?)")->execute([$name, $slug, $parent_id, $image, $sort_order]);
        }
        header("Location: admin.php?view=categories&msg=" . urlencode("دسته‌بندی ذخیره شد."));
        exit;
    }
    ...
    if ($action === 'save_menu') {
        ...
        if ($id) {
            $db->prepare("UPDATE menus SET title=?, link=?, parent_id=?, badge_text=?, badge_color=?, is_mega=?, mega_content=?, sort_order=? WHERE id=?")->execute([$title, $link, $parent_id, $badge_text, $badge_color, $is_mega, $mega_content, $sort_order, $id]);
        } else {
            $db->prepare("INSERT INTO menus (title, link, parent_id, badge_text, badge_color, is_mega, mega_content, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")->execute([$title, $link, $parent_id, $badge_text, $badge_color, $is_mega, $mega_content, $sort_order]);
        }
        header("Location: admin.php?view=menus&msg=" . urlencode("آیتم فهرست ذخیره شد."));
        exit;
    }
    ...
    if ($action === 'save_slider') {
        ...
        if ($id) {
            $db->prepare("UPDATE sliders SET title=?, image=?, link=?, sort_order=? WHERE id=?")->execute([$title, $image, $link, $sort_order, $id]);
        } else {
            $db->prepare("INSERT INTO sliders (title, image, link, sort_order) VALUES (?, ?, ?, ?)")->execute([$title, $image, $link, $sort_order]);
        }
        header("Location: admin.php?view=sliders&msg=" . urlencode("اسلاید ذخیره شد."));
        exit;
    }
    ...
    if ($action === 'save_banner') {
        ...
        if ($id) {
            $db->prepare("UPDATE banners SET title=?, image=?, link=?, position=?, sort_order=? WHERE id=?")->execute([$title, $image, $link, $position, $sort_order, $id]);
        } else {
            $db->prepare("INSERT INTO banners (title, image, link, position, sort_order) VALUES (?, ?, ?, ?, ?)")->execute([$title, $image, $link, $position, $sort_order]);
        }
        header("Location: admin.php?view=sliders&msg=" . urlencode("بنر ذخیره شد."));
        exit;
    }
    ...
    if ($action === 'save_post') {
        $id = !empty($_POST['id']) ? intval($_POST['id']) : null;
        $title = trim($_POST['title'] ?? '');
        $slug = trim($_POST['slug'] ?? '') ?: 'post-' . time();
        ...
        if ($id) {
            $db->prepare("UPDATE posts SET title=?, slug=?, author=?, image=?, excerpt=?, content=? WHERE id=?")->execute([$title, $slug, $author, $image, $excerpt, $content, $id]);
        } else {
            $db->prepare("INSERT INTO posts (title, slug, author, image, excerpt, content) VALUES (?, ?, ?, ?, ?, ?)")->execute([$title, $slug, $author, $image, $excerpt, $content]);
        }
        header("Location: admin.php?view=posts&msg=" . urlencode("مقاله ذخیره شد."));
        exit;
    }
    ...
    if ($action === 'save_page') {
        $id = !empty($_POST['id']) ? intval($_POST['id']) : null;
        $title = trim($_POST['title'] ?? '');
        $slug = trim($_POST['slug'] ?? '') ?: 'page-' . time();
        $content = $_POST['content'] ?? '';
        $is_published = isset($_POST['is_published']) ? 1 : 0;

        if ($id) {
            $db->prepare("UPDATE pages SET title=?, slug=?, content=?, is_published=? WHERE id=?")->execute([$title, $slug, $content, $is_published, $id]);
        } else {
            $db->prepare("INSERT INTO pages (title, slug, content, is_published) VALUES (?, ?, ?, ?)")->execute([$title, $slug, $content, $is_published]);
        }
        header("Location: admin.php?view=pages&msg=" . urlencode("برگه ذخیره شد."));
        exit;
    }
    ...
    if ($action === 'reply_comment') {
        ...
        if ($id) {
            $db->prepare("UPDATE comments SET reply_text=?, status='approved' WHERE id=?")->execute([$reply, $id]);
        }
        header("Location: admin.php?view=comments&msg=" . urlencode("پاسخ دیدگاه ثبت شد."));
        exit;
    }

    if ($action === 'import_json' && !empty($_FILES['backup_file']['tmp_name'])) {
        $content = file_get_contents($_FILES['backup_file']['tmp_name']);
        $data = json_decode($content, true);
        if ($data && is_array($data)) {
            if (!empty($data['settings'])) {
                foreach ($data['settings'] as $row) {
                    $db->prepare("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)")->execute([$row['key'], $row['value']]);
                }
            }
            if (!empty($data['products'])) {
                foreach ($data['products'] as $p) {
                    $db->prepare("INSERT OR REPLACE INTO products (id, slug, title, category_slug, price, old_price, image, gallery, description, specs, variables, external_link, is_featured, is_special, views) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
                       ->execute([$p['id'], $p['slug'], $p['title'], $p['category_slug'], $p['price'], $p['old_price'], $p['image'], $p['gallery'], $p['description'], $p['specs'], $p['variables'] ?? '', $p['external_link'], $p['is_featured'], $p['is_special'], $p['views']]);
                }
            }
            header("Location: admin.php?view=import_export&msg=" . urlencode("درون‌ریزی بکاپ انجام شد."));
            exit;
        }
    }
}
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Peyda', Tahoma, sans-serif; }
        ...
        /* نوار ابزار غنی داخلی و نوار تغییر حالت ویرایشگر */
        .rich-toolbar {
            display: flex;
            gap: 6px;
            flex-wrap: wrap;
...
<body>
        ...
        <div class="content-body">
            ...
            <?php if ($view === 'dashboard'): ?>
                ...
                <div class="stats-grid">
                    ...
                    <div class="stat-card">
                        <div class="stat-icon" style="background:#ec4899;"><span class="material-symbols-outlined">category</span></div>
                        <div class="stat-info">
                    ...
                    <div class="stat-card">
                        <div class="stat-icon" style="background:#22c55e;"><span class="material-symbols-outlined">article</span></div>
                        <div class="stat-info">
                            <h3><?= $posts_count ?></h3>
                    ...
                    <div class="stat-card">
                        <div class="stat-icon" style="background:#3b82f6;"><span class="material-symbols-outlined">comment</span></div>
                        <div class="stat-info">
                            <h3><?= $comments_count ?></h3>
                            <span>دیدگاه‌های ثبت‌شده</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    ...
                    <?php
                    $latest_p = $db->query("SELECT * FROM products ORDER BY id DESC LIMIT 6")->fetchAll();
                    if (empty($latest_p)):
                    ?>
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    <?php foreach ($latest_p as $p): ?>
                                        <tr>
                                            <td><img src="<?= htmlspecialchars($p['image']) ?>" alt="" style="width:42px;height:42px;border-radius:8px;object-fit:cover;"></td>
                                            <td><strong><?= htmlspecialchars($p['title']) ?></strong></td>
                                            <td><?= htmlspecialchars($p['category_slug']) ?></td>
                                            <td style="color:var(--primary); font-weight:800;"><?= number_format($p['price']) ?></td>
                                            <td>
                                                <?php if ($p['external_link']): ?>
                                                    <a href="<?= htmlspecialchars($p['external_link']) ?>" target="_blank" style="color:var(--primary);">مشاهده لینک ↗</a>
                                                <?php else: ?>
                                                    <span style="color:var(--muted);">-</span>
                                                <?php endif; ?>
                                            </td>
                                            <td>
                                                <a href="index.php?product=<?= $p['slug'] ?>" target="_blank" class="btn btn-sm" style="background:var(--bg); border:1px solid var(--border);">نمایش</a>
                                                <a href="admin.php?view=edit_product&id=<?= $p['id'] ?>" class="btn btn-sm btn-primary">ویرایش</a>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                        ...
                        <table class="table">
                            <thead>
                                <tr>
                                    ...
                                    <th>دسته</th>
                                    <th>قیمت (ریال)</th>
                                    <th>لینک خرید در سایت فروشنده</th>
                                    <th>پیشنهاد شگفت‌انگیز</th>
                                    <th>عملیات</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php
                                $prods = $db->query("SELECT * FROM products ORDER BY id DESC")->fetchAll();
                                foreach ($prods as $p):
                                ?>
                                    <tr>
                                        <td><img src="<?= htmlspecialchars($p['image']) ?>" alt="" style="width:44px;height:44px;border-radius:8px;object-fit:cover;"></td>
                                        <td><strong><?= htmlspecialchars($p['title']) ?></strong></td>
                                        <td><?= htmlspecialchars($p['category_slug']) ?></td>
                                        <td style="color:var(--primary); font-weight:800;"><?= number_format($p['price']) ?></td>
                                        <td>
                                            <?php if ($p['external_link']): ?>
                                                <a href="<?= htmlspecialchars($p['external_link']) ?>" target="_blank" style="color:var(--primary); font-weight:700;">لینک خرید ↗</a>
                                            <?php else: ?>
                                                <span style="color:var(--muted);">-</span>
                                            <?php endif; ?>
                                        </td>
                                        <td><?= $p['is_special'] ? '<span style="color:#ef4444;font-weight:800;">بله</span>' : 'خیر' ?></td>
                                        <td>
                                            <a href="index.php?product=<?= $p['slug'] ?>" target="_blank" class="btn btn-sm" style="background:var(--bg); border:1px solid var(--border);">نمایش</a>
                                            <a href="admin.php?view=edit_product&id=<?= $p['id'] ?>" class="btn btn-sm btn-primary">ویرایش</a>
                                            <a href="admin.php?action=del_product&id=<?= $p['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('حذف شود؟')">حذف</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
            ...
            <?php elseif ($view === 'edit_product'): 
                $prod = [
                    ...
                    'price' => '', 'old_price' => '', 'image' => '', 'gallery' => '',
                    'description' => '', 'specs' => '', 'variables' => '', 'external_link' => '',
                    'is_featured' => 1, 'is_special' => 0
                ];
                if (isset($_GET['id'])) {
                ...
                $gallery_list = json_decode($prod['gallery'], true) ?: [];
                $specs_arr = json_decode($prod['specs'], true) ?: [];
                $specs_text = '';
                foreach ($specs_arr as $k => $v) { $specs_text .= $k . ':' . $v . "\n"; }
                    ...
                    <div style="display:grid; grid-template-columns: 2.2fr 1fr; gap:24px;">
                        <div>
                            <div class="card">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="title" class="form-control" value="<?= htmlspecialchars($prod['title']) ?>" required>
                                </div>

                                <!-- ویرایشگر دوحالته: بصری (TinyMCE) / کد HTML خالص برای درج و ویرایش IFrame و اچ‌تی‌ام‌ال -->
                                <div class="form-group">
                            ...
                            <div class="card">
                                ...
                                <p style="color:var(--muted); font-size:13px; margin-bottom:16px;">
                                    متغیرهای محصول (مانند رنگ مشکی، سفید یا حافظه ۲۵۶ گیگابایت) را تعریف کنید. کاربر با کلیک روی هر رنگ، تصویر اصلی گالری را به تصویر آن متغیر تغییر می‌دهد:
                                </p>
                                <textarea name="variables_json" rows="8" class="form-control" style="direction:ltr;text-align:left;font-family:monospace;" placeholder='[{"group_name":"رنگ","items":[{"name":"مشکی","hex":"#000000","image":"https://..."},{"name":"سفید","hex":"#ffffff","image":"https://..."}]}]'><?= htmlspecialchars(json_encode($variables_arr, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE)) ?></textarea>
                            </div>

                            <div class="card">
                                <div class="card-title">مشخصات فنی محصول</div>
                                <div class="form-group">
                                    <label>هر ویژگی را در یک خط با فرمت <code>عنوان:مقدار</code> بنویسید (مثال: برند:اپل)</label>
                                    <textarea name="specs_raw" rows="5" class="form-control" placeholder="برند:اپل&#10;رنگ:مشکی"><?= htmlspecialchars(trim($specs_text)) ?></textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            <div class="card">
                                ...
                                <div class="form-group">
                                    <label style="color:var(--primary); font-weight:800;">لینک صفحه خرید محصول در سایت فروشنده (URL)</label>
                                    <input type="text" name="external_link" class="form-control" value="<?= htmlspecialchars($prod['external_link']) ?>" placeholder="https://...">
                                    <span style="font-size:12px; color:var(--muted);">کاربر با کلیک روی دکمه خرید مستقیماً به این آدرس هدایت می‌شود.</span>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    <label>قیمت خط‌خورده (ریال)</label>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="category_slug" class="form-control">
                                        ...
                                        foreach ($cats as $c):
                                        ?>
                                            <option value="<?= $c['slug'] ?>" <?= $prod['category_slug'] == $c['slug'] ? 'selected' : '' ?>><?= htmlspecialchars($c['name']) ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label>نامک انگلیسی (Slug)</label>
                                    <input type="text" name="slug" class="form-control" value="<?= htmlspecialchars($prod['slug']) ?>" placeholder="product-slug">
                                </div>
                                <div style="display:flex; flex-direction:column; gap:10px; margin: 16px 0;">
                                    <label style="display:flex; align-items:center; gap:8px; cursor:pointer;">
                                        <input type="checkbox" name="is_special" value="1" <?= $prod['is_special'] ? 'checked' : '' ?>>
                                        <span style="color:#ef4444; font-weight:800;">نمایش در پیشنهاد شگفت‌انگیز</span>
                                    </label>
                                    <label style="display:flex; align-items:center; gap:8px; cursor:pointer;">
                            ...
                            <div class="card">
                                ...
                                <div class="form-group">
                                    <label>آدرس تصویر اصلی (URL)</label>
                                    <input type="text" name="image" class="form-control" value="<?= htmlspecialchars($prod['image']) ?>" required>
                                </div>
                                <div class="form-group">
                                    <label>آدرس تصاویر گالری (هر تصویر در یک خط)</label>
                                    <textarea name="gallery" rows="4" class="form-control"><?= htmlspecialchars(implode("\n", $gallery_list)) ?></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <!-- ---------------------------------------------------------------
                 3. دسته‌بندی‌ها (بدون آیکون، با تصویر)
            --------------------------------------------------------------- -->
            <?php elseif ($view === 'categories'): ?>
                <div class="page-header">
                    <h1>مدیریت دسته‌بندی‌ها (همراه با تصویر)</h1>
                </div>
                    ...
                    <div class="card">
                        ...
                        <form method="POST" action="admin.php">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="name" class="form-control" required placeholder="کالای دیجیتال">
                            </div>
                            <div class="form-group">
                                <label>نامک انگلیسی (Slug)</label>
                                <input type="text" name="slug" class="form-control" required placeholder="digital">
                            </div>
                            <div class="form-group">
                                <label>آدرس تصویر دسته‌بندی (URL)</label>
                            ...
                            <tbody>
                                ...
                                ?>
                                    <tr>
                                        <td>
                                            <?php if ($c['image']): ?>
                                                <img src="<?= htmlspecialchars($c['image']) ?>" alt="" style="width:40px;height:40px;border-radius:50%;object-fit:cover;">
                                            <?php else: ?>
                                                <span style="color:var(--muted);">-</span>
                                            <?php endif; ?>
                                        </td>
                                        ...
                                        <td>
                                            <a href="index.php?route=shop&cat=<?= $c['slug'] ?>" target="_blank" class="btn btn-sm" style="background:var(--bg); border:1px solid var(--border);">نمایش</a>
                                            <a href="admin.php?action=del_category&id=<?= $c['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('حذف شود؟')">حذف</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
            ...
            <!-- ---------------------------------------------------------------
                 4. فهرست‌ها و سازنده گرافیکی مگامنو (Graphical Mega Menu Builder)
            --------------------------------------------------------------- -->
            ...
            ?>
                <div class="page-header">
                    ...
                    <?php if ($menu_item['id']): ?>
                        <a href="admin.php?view=menus" class="btn" style="background:var(--card-bg); border:1px solid var(--border);">← بازگشت</a>
                    <?php endif; ?>
                </div>
                    ...
                    <div class="card">
                        ...
                        <form method="POST" action="admin.php" onsubmit="serializeGraphicalMegaMenu()">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="title" class="form-control" value="<?= htmlspecialchars($menu_item['title']) ?>" required placeholder="فروشگاه">
                            </div>
                            <div class="form-group">
                                <label>لینک</label>
                                <input type="text" name="link" class="form-control" value="<?= htmlspecialchars($menu_item['link']) ?>" required placeholder="?route=shop">
                            </div>
                            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:12px;">
                                <div class="form-group">
                                    <label>برچسب رنگی (اختیاری)</label>
                                    <input type="text" name="badge_text" class="form-control" value="<?= htmlspecialchars($menu_item['badge_text']) ?>" placeholder="ویژه / تخفیف">
                                </div>
                                <div class="form-group">
                                    <label>رنگ برچسب</label>
                                    <input type="color" name="badge_color" class="form-control" value="<?= htmlspecialchars($menu_item['badge_color'] ?: '#ef4444') ?>" style="height:44px;padding:4px;">
                                </div>
                            </div>
                            <div class="form-group">
                                <label style="display:flex; align-items:center; gap:8px; cursor:pointer;">
                                    <input type="checkbox" name="is_mega" id="is-mega-chk" value="1" <?= $menu_item['is_mega'] ? 'checked' : '' ?> onchange="toggleGraphicalMegaBuilder()">
                                    <span style="font-weight:800; color:var(--primary);">فعال‌سازی مگامنو (طراحی گرافیکی ستون‌ها و بنر)</span>
                                </label>
                            </div>

                            <!-- سازنده گرافیکی مگامنو بدون نیاز به کدنویسی JSON -->
                            <div id="graphical-mega-builder" style="display:<?= $menu_item['is_mega'] ? 'block' : 'none' ?>; margin: 18px 0; border-top: 1px dashed var(--border); padding-top: 16px;">
                                ...
                                <div id="mega-columns-list">
                                    <?php foreach ($mega_decoded['columns'] as $c_idx => $col): ?>
                                        <div class="mega-builder-col">
                                            <div style="display:flex; justify-content:space-between; margin-bottom:10px;">
                                                <input type="text" class="form-control col-title-input" value="<?= htmlspecialchars($col['title']) ?>" placeholder="عنوان ستون (مثال: دیجیتال)" style="width:75%;">
                                            ...
                                            <div class="col-links-list">
                                                <?php foreach ($col['links'] as $link_item): ?>
                                                    <div class="mega-link-row">
                                                        <input type="text" class="form-control link-title-input" value="<?= htmlspecialchars($link_item['title']) ?>" placeholder="نام لینک" style="width:45%;">
                                                        <input type="text" class="form-control link-url-input" value="<?= htmlspecialchars($link_item['url']) ?>" placeholder="آدرس لینک" style="width:45%;">
                                ...
                                <div style="margin-top:20px; border-top: 1px dashed var(--border); padding-top:14px;">
                                    ...
                                    <input type="text" id="promo-title-input" class="form-control" value="<?= htmlspecialchars($mega_decoded['promo']['title'] ?? '') ?>" placeholder="عنوان بنر (جشنواره اپل)" style="margin-bottom:8px;">
                                    <input type="text" id="promo-image-input" class="form-control" value="<?= htmlspecialchars($mega_decoded['promo']['image'] ?? '') ?>" placeholder="آدرس تصویر بنر (URL)" style="margin-bottom:8px;">
                            ...
                            <tbody>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td><code><?= htmlspecialchars($m['link']) ?></code></td>
                                        <td><?= $m['badge_text'] ? '<span style="background:'.htmlspecialchars($m['badge_color']).';color:#fff;padding:2px 8px;border-radius:99px;font-size:11px;">'.htmlspecialchars($m['badge_text']).'</span>' : '-' ?></td>
                                        <td><?= $m['is_mega'] ? '<span style="color:#22c55e;font-weight:800;">فعال</span>' : 'ساده' ?></td>
                                        <td>
                                            <a href="admin.php?view=edit_menu&id=<?= $m['id'] ?>" class="btn btn-sm btn-primary">ویرایش</a>
                                            <a href="admin.php?action=del_menu&id=<?= $m['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('حذف شود؟')">حذف</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
            ...
            <?php elseif ($view === 'sliders'): ?>
                <div class="page-header">
                    <h1>مدیریت اسلایدها و بنرها (فقط تصویر و لینک)</h1>
                </div>
                ...
                <div style="display:grid; grid-template-columns: 1fr 2fr; gap:24px; margin-bottom:30px;">
                    <div class="card">
                        ...
                        <form method="POST" action="admin.php">
                            ...
                            <div class="form-group">
                                <label>عنوان داخلی (برای مدیریت)</label>
                                <input type="text" name="title" class="form-control" required placeholder="اسلاید جشنواره">
                            </div>
                            <div class="form-group">
                                <label>آدرس تصویر بنر (URL)</label>
                                <input type="text" name="image" class="form-control" required>
                            </div>
                            <div class="form-group">
                                <label>لینک مقصد با کلیک روی اسلاید</label>
                                <input type="text" name="link" class="form-control" value="?route=shop">
                            </div>
                    ...
                    <div class="card">
                        <div class="card-title">اسلاید‌های فعال (بدون اورلی و متن روی تصویر)</div>
                        <table class="table">
                            <thead>
                                <tr>
                                    <th>تصویر</th>
                            ...
                            <tbody>
                                <?php
                                $slides = $db->query("SELECT * FROM sliders ORDER BY id ASC")->fetchAll();
                                foreach ($slides as $s):
                                ?>
                                    <tr>
                                        <td><img src="<?= htmlspecialchars($s['image']) ?>" alt="" style="width:70px;height:40px;object-fit:cover;border-radius:6px;"></td>
                                        <td><strong><?= htmlspecialchars($s['title']) ?></strong></td>
                                        <td><code><?= htmlspecialchars($s['link']) ?></code></td>
                                        <td>
                                            <a href="admin.php?action=del_slider&id=<?= $s['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('حذف شود؟')">حذف</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                ...
                <div style="display:grid; grid-template-columns: 1fr 2fr; gap:24px;">
                    <div class="card">
                        ...
                        <form method="POST" action="admin.php">
                            ...
                            <div class="form-group">
                                <label>موقعیت نمایش در سایت</label>
                                <select name="position" class="form-control">
                                    <option value="hero_side">کنار اسلایدر اصلی (hero_side)</option>
                                    <option value="home_middle">بنرهای تبلیغاتی وسط صفحه اصلی (home_middle)</option>
                                </select>
                            </div>
                    ...
                    <div class="card">
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                <?php
                                $banners = $db->query("SELECT * FROM banners ORDER BY id ASC")->fetchAll();
                                foreach ($banners as $b):
                                ?>
                                    <tr>
                                        <td><img src="<?= htmlspecialchars($b['image']) ?>" alt="" style="width:70px;height:40px;object-fit:cover;border-radius:6px;"></td>
                                        <td><strong><?= htmlspecialchars($b['title']) ?></strong></td>
                                        <td><?= $b['position'] == 'home_middle' ? '<span style="color:#ef4444;font-weight:700;">وسط صفحه اصلی</span>' : 'کنار اسلایدر' ?></td>
                                        <td>
                                            <a href="admin.php?action=del_banner&id=<?= $b['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('حذف شود؟')">حذف</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
            ...
            <?php elseif ($view === 'posts'): ?>
                <div class="page-header">
                    <h1>مقالات وبلاگ</h1>
                    <a href="admin.php?view=edit_post" class="btn btn-primary"><span class="material-symbols-outlined" style="font-size:18px;">add</span> نوشتن مقاله جدید</a>
                </div>
                ...
                <div class="card">
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    <td><img src="<?= htmlspecialchars($po['image']) ?>" alt="" style="width:50px;height:36px;border-radius:6px;object-fit:cover;"></td>
                                    <td><strong><?= htmlspecialchars($po['title']) ?></strong></td>
                                    <td><?= htmlspecialchars($po['author']) ?></td>
                                    <td><?= $po['views'] ?></td>
                                    <td>
                                        <a href="index.php?post=<?= $po['slug'] ?>" target="_blank" class="btn btn-sm" style="background:var(--bg); border:1px solid var(--border);">نمایش</a>
                                        <a href="admin.php?view=edit_post&id=<?= $po['id'] ?>" class="btn btn-sm btn-primary">ویرایش</a>
                                        <a href="admin.php?action=del_post&id=<?= $po['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('حذف شود؟')">حذف</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
            ...
            <?php elseif ($view === 'pages'): ?>
                <div class="page-header">
                    <h1>مدیریت برگه‌های سایت</h1>
                    <a href="admin.php?view=edit_page" class="btn btn-primary"><span class="material-symbols-outlined" style="font-size:18px;">add</span> ایجاد برگه جدید</a>
                </div>
                ...
                <div class="card">
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    <td><strong><?= htmlspecialchars($pa['title']) ?></strong></td>
                                    <td><code>?page=<?= htmlspecialchars($pa['slug']) ?></code></td>
                                    <td>
                                        <a href="index.php?page=<?= $pa['slug'] ?>" target="_blank" class="btn btn-sm" style="background:var(--bg); border:1px solid var(--border);">نمایش</a>
                                        <a href="admin.php?view=edit_page&id=<?= $pa['id'] ?>" class="btn btn-sm btn-primary">ویرایش</a>
                                        <a href="admin.php?action=del_page&id=<?= $pa['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('حذف شود؟')">حذف</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                ...
                <form method="POST" action="admin.php">
                    ...
                    <div class="card">
                        <div class="form-group">
                            ...
                            <input type="text" name="title" class="form-control" value="<?= htmlspecialchars($page_row['title']) ?>" required>
                        </div>
                        <div class="form-group">
                            <label>نامک انگلیسی (Slug)</label>
                            <input type="text" name="slug" class="form-control" value="<?= htmlspecialchars($page_row['slug']) ?>" placeholder="about">
                        </div>
                        <!-- ویرایشگر دوحالته برگه -->
                        <div class="form-group">
            ...
            <!-- ---------------------------------------------------------------
                 8. مدیریت دیدگاه‌ها و نظرات
            --------------------------------------------------------------- -->
            <?php elseif ($view === 'comments'): ?>
                <div class="page-header">
                    <h1>مدیریت دیدگاه‌ها و نظرات کاربران</h1>
                </div>
                ...
                <div class="card">
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    <td><strong><?= htmlspecialchars($cm['name']) ?></strong></td>
                                    <td><code><?= htmlspecialchars($cm['item_slug']) ?></code></td>
                                    <td><?= htmlspecialchars($cm['comment']) ?></td>
                                    <td><span style="color:#f59e0b;"><?= str_repeat('★', $cm['rating']) ?></span></td>
                                    <td>
                                        <?php if (!empty($cm['reply_text'])): ?>
                                            <span style="color:var(--primary); font-weight:700;"><?= htmlspecialchars($cm['reply_text']) ?></span>
                                        <?php else: ?>
                                            <span style="color:var(--muted);">-</span>
                                        <?php endif; ?>
                                    </td>
                                    <td style="min-width:160px;">
                                        <button onclick="openReplyModal(<?= $cm['id'] ?>, '<?= htmlspecialchars(addslashes($cm['name'])) ?>')" class="btn btn-sm btn-primary">پاسخ</button>
                                        <a href="admin.php?action=del_comment&id=<?= $cm['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('حذف شود؟')">حذف</a>
                                    </td>
                                </tr>
                ...
                <div id="reply-modal" style="position:fixed; top:0; left:0; right:0; bottom:0; background:rgba(0,0,0,0.6); display:none; align-items:center; justify-content:center; z-index:9999;">
                    <div style="background:var(--card-bg); padding:30px; border-radius:20px; max-width:440px; width:100%; border:1px solid var(--border); box-shadow:var(--shadow);">
            ...
            <!-- ---------------------------------------------------------------
                 9. اسکریپت‌ها و نمادها
            --------------------------------------------------------------- -->
            <?php elseif ($view === 'scripts'): ?>
                <div class="page-header">
                    <h1>اسکریپت‌ها، کدها و نمادهای رسمی (اینماد)</h1>
                </div>
                ...
                <form method="POST" action="admin.php">
                    ...
                    <div class="card">
                        <div class="card-title">اسکریپت‌های هدر (&lt;head&gt;)</div>
                        <textarea name="head_scripts" rows="4" class="form-control" style="direction:ltr;text-align:left;font-family:monospace;"><?= htmlspecialchars($settings['head_scripts']) ?></textarea>
                    </div>

                    <div class="card">
                        <div class="card-title">اسکریپت‌های ابتدای بدنه (&lt;body&gt;)</div>
                        <textarea name="body_scripts" rows="4" class="form-control" style="direction:ltr;text-align:left;font-family:monospace;"><?= htmlspecialchars($settings['body_scripts']) ?></textarea>
                    </div>

                    <div class="card">
                        <div class="card-title">اسکریپت‌های انتهای صفحه (&lt;/body&gt;)</div>
                        <textarea name="footer_scripts" rows="4" class="form-control" style="direction:ltr;text-align:left;font-family:monospace;"><?= htmlspecialchars($settings['footer_scripts']) ?></textarea>
                    </div>

                    <div class="card">
                        <div class="card-title">کدهای نماد اعتماد الکترونیکی (اینماد) و ساماندهی در فوتر</div>
                        <textarea name="trust_badges" rows="4" class="form-control" placeholder="<div...>کد نماد...</div>"><?= htmlspecialchars($settings['trust_badges']) ?></textarea>
                    </div>
            ...
            <!-- ---------------------------------------------------------------
                 10. درون‌ریزی و برون‌بری
            --------------------------------------------------------------- -->
            <?php elseif ($view === 'import_export'): ?>
                <div class="page-header">
                    <h1>درون‌ریزی و برون‌بری اطلاعات</h1>
                </div>
                ...
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:24px;">
                    <div class="card">
                        ...
                        <p style="color:var(--muted); margin-bottom:18px;">
                            دریافت فایل پشتیبان کامل از تمامی جداول دیتابیس اریا استور با فرمت JSON.
                        </p>
                        <div style="display:flex; gap:10px;">
                    ...
                    <div class="card">
                        ...
                        <p style="color:var(--muted); margin-bottom:18px;">
                            آپلود فایل بکاپ JSON برای بازیابی اطلاعات در پایگاه داده.
                        </p>
            ...
            <!-- ---------------------------------------------------------------
                 11. تنظیمات سایت، چیدمان، فوتر و شبکه‌های اجتماعی دلخواه
            --------------------------------------------------------------- -->
            <?php elseif ($view === 'settings'): ?>
                <div class="page-header">
                    <h1>تنظیمات سایت، چیدمان، هدر، فوتر و شبکه‌های اجتماعی دلخواه</h1>
                </div>
                ...
                <form method="POST" action="admin.php" onsubmit="serializeCustomSocialNetworks()">
                    ...
                    <div class="card">
                        ...
                        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:16px;">
                            <div class="form-group">
                                <label>عنوان اصلی سایت (Site Title)</label>
                                <input type="text" name="site_title" class="form-control" value="<?= htmlspecialchars($settings['site_title']) ?>" required>
                            </div>
                            <div class="form-group">
                                <label>شعار یا توضیح کوتاه سایت</label>
                                <input type="text" name="site_subtitle" class="form-control" value="<?= htmlspecialchars($settings['site_subtitle']) ?>">
                            </div>
                        </div>
                        <div style="display:grid; grid-template-columns: 1.5fr 1fr 1fr; gap:16px;">
                            <div class="form-group">
                                <label>آدرس تصویر لوگوی هدر و ادمین (URL)</label>
                                <input type="text" name="logo_url" class="form-control" value="<?= htmlspecialchars($settings['logo_url']) ?>" placeholder="https://...">
                            </div>
                            <div class="form-group">
                                <label>متن نوار بالایی هدر (Top Bar)</label>
                                <input type="text" name="top_bar_text" class="form-control" value="<?= htmlspecialchars($settings['top_bar_text']) ?>">
                            </div>
                            <div class="form-group">
                                <label>شماره تماس نوار بالایی</label>
                                <input type="text" name="top_bar_phone" class="form-control" value="<?= htmlspecialchars($settings['top_bar_phone']) ?>">
                            </div>
                        </div>
                    </div>

                    <!-- کارت ۲: کنترل کامل چیدمان و بخش‌های صفحه اصلی -->
                    <div class="card">
                        <div class="card-title"><span class="material-symbols-outlined" style="color:var(--primary);">dashboard</span> کنترل نمایش و عناوین بخش‌های صفحه اصلی (Home Page Sections)</div>
                        <div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:18px;">
                            <div class="form-group">
                                <label>نمایش اسلایدر اصلی هدر</label>
                                <select name="show_hero_slider" class="form-control">
                                    <option value="1" <?= ($settings['show_hero_slider'] ?? '1') == '1' ? 'selected' : '' ?>>فعال (نمایش)</option>
                                    <option value="0" <?= ($settings['show_hero_slider'] ?? '1') == '0' ? 'selected' : '' ?>>غیرفعال (مخفی)</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label>نمایش دسته‌بندی‌های محبوب</label>
                                <select name="show_categories" class="form-control">
                                    <option value="1" <?= ($settings['show_categories'] ?? '1') == '1' ? 'selected' : '' ?>>فعال (نمایش)</option>
                                    <option value="0" <?= ($settings['show_categories'] ?? '1') == '0' ? 'selected' : '' ?>>غیرفعال (مخفی)</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label>عنوان بخش دسته‌بندی‌ها</label>
                                <input type="text" name="categories_title" class="form-control" value="<?= htmlspecialchars($settings['categories_title'] ?? 'دسته‌بندی‌های محبوب محصولات') ?>">
                            </div>

                            <div class="form-group">
                                <label>نمایش بنرهای میانی صفحه اصلی</label>
                                <select name="show_middle_banners" class="form-control">
                                    <option value="1" <?= ($settings['show_middle_banners'] ?? '1') == '1' ? 'selected' : '' ?>>فعال (نمایش)</option>
                                    <option value="0" <?= ($settings['show_middle_banners'] ?? '1') == '0' ? 'selected' : '' ?>>غیرفعال (مخفی)</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label>نمایش پیشنهاد شگفت‌انگیز</label>
                                <select name="show_special_offers" class="form-control">
                                    <option value="1" <?= ($settings['show_special_offers'] ?? '1') == '1' ? 'selected' : '' ?>>فعال (نمایش)</option>
                                    <option value="0" <?= ($settings['show_special_offers'] ?? '1') == '0' ? 'selected' : '' ?>>غیرفعال (مخفی)</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label>عنوان بخش پیشنهاد شگفت‌انگیز</label>
                                <input type="text" name="special_offers_title" class="form-control" value="<?= htmlspecialchars($settings['special_offers_title'] ?? 'پیشنهاد شگفت‌انگیز') ?>">
                            </div>

                            <div class="form-group">
                                <label>نمایش جدیدترین محصولات</label>
                                <select name="show_latest_products" class="form-control">
                                    <option value="1" <?= ($settings['show_latest_products'] ?? '1') == '1' ? 'selected' : '' ?>>فعال (نمایش)</option>
                                    <option value="0" <?= ($settings['show_latest_products'] ?? '1') == '0' ? 'selected' : '' ?>>غیرفعال (مخفی)</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label>عنوان بخش جدیدترین محصولات</label>
                                <input type="text" name="latest_products_title" class="form-control" value="<?= htmlspecialchars($settings['latest_products_title'] ?? 'جدیدترین محصولات') ?>">
                            </div>
                            <div class="form-group">
                                <label>نمایش اسلایدر مقالات وبلاگ</label>
                                <select name="show_blog_slider" class="form-control">
                                    <option value="1" <?= ($settings['show_blog_slider'] ?? '1') == '1' ? 'selected' : '' ?>>فعال (نمایش)</option>
                                    <option value="0" <?= ($settings['show_blog_slider'] ?? '1') == '0' ? 'selected' : '' ?>>غیرفعال (مخفی)</option>
                                </select>
                            </div>
                        </div>
                    </div>

                    <!-- کارت ۳: تنظیمات فوتر ۴ ستونه همراه با مدیریت لینک‌های فوتر -->
                    <div class="card">
                        <div class="card-title"><span class="material-symbols-outlined" style="color:var(--primary);">view_column</span> کنترل ستون‌ها، لینک‌ها و متون فوتر ۴ ستونه سایت</div>
                        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:16px;">
                            <div class="form-group">
                                <label>عنوان ستون اول فوتر (معرفی سایت)</label>
                                <input type="text" name="footer_col1_title" class="form-control" value="<?= htmlspecialchars($settings['footer_col1_title'] ?? 'معرفی اریا استور') ?>">
                            </div>
                            <div class="form-group">
                                <label>عنوان ستون دوم فوتر</label>
                                <input type="text" name="footer_col2_title" class="form-control" value="<?= htmlspecialchars($settings['footer_col2_title'] ?? 'دسترسی سریع') ?>">
                            </div>
                            <div class="form-group">
                                <label>عنوان ستون سوم فوتر</label>
                                <input type="text" name="footer_col3_title" class="form-control" value="<?= htmlspecialchars($settings['footer_col3_title'] ?? 'راهنمای کاربران') ?>">
                            </div>
                            <div class="form-group">
                                <label>عنوان ستون چهارم فوتر (نمادها)</label>
                                <input type="text" name="footer_col4_title" class="form-control" value="<?= htmlspecialchars($settings['footer_col4_title'] ?? 'نمادهای رسمی و اعتماد') ?>">
                            </div>
                        </div>
                        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:16px;">
                            <div class="form-group">
                                <label>لینک‌های ستون دوم فوتر (هر لینک در یک خط با فرمت <code>عنوان|لینک</code>)</label>
                                <textarea name="footer_col2_links" rows="5" class="form-control" placeholder="صفحه اصلی|?&#10;فروشگاه|?route=shop"><?= htmlspecialchars($settings['footer_col2_links'] ?? "صفحه اصلی|?\nهمه محصولات|?route=shop\nلیست علاقه‌مندی‌ها|?route=wishlist\nمقالات و آموزش‌ها|?route=blog") ?></textarea>
                            </div>
                            <div class="form-group">
                                <label>لینک‌های ستون سوم فوتر (هر لینک در یک خط با فرمت <code>عنوان|لینک</code>)</label>
                                <textarea name="footer_col3_links" rows="5" class="form-control" placeholder="درباره ما|?page=about&#10;تماس با ما|?page=contact"><?= htmlspecialchars($settings['footer_col3_links'] ?? "درباره ما|?page=about\nتماس با ما|?page=contact\nرویه بررسی کالا|#\nحریم خصوصی کاربران|#") ?></textarea>
                            </div>
                        </div>
                        <div class="form-group">
                            <label>متن معرفی فروشگاه در ستون اول فوتر</label>
                            <textarea name="footer_about" rows="3" class="form-control"><?= htmlspecialchars($settings['footer_about']) ?></textarea>
                        </div>
                        <div class="form-group">
                            <label>متن کپی‌رایت پایین صفحه</label>
                            <input type="text" name="footer_copyright" class="form-control" value="<?= htmlspecialchars($settings['footer_copyright']) ?>">
                        </div>
                    </div>

                    <!-- کارت ۴: مدیریت شبکه‌های اجتماعی دلخواه (ساخت شبکه جدید همراه با آیکون PNG) -->
                    <div class="card">
                        <div class="card-title"><span class="material-symbols-outlined" style="color:var(--primary);">share</span> مدیریت شبکه‌های اجتماعی (ساخت شبکه اجتماعی دلخواه همراه با آیکون PNG)</div>
                        <p style="color:var(--muted); font-size:13px; margin-bottom:16px;">
                            شما می‌توانید هر تعداد شبکه اجتماعی دلخواه (اینستاگرام، تلگرام، توییتر، انجمن اختصاصی و...) را همراه با آدرس لینک و آدرس تصویر آیکون (PNG) تعریف کنید:
                        </p>

                        <div id="social-networks-builder">
                            <?php
                            $custom_socials = json_decode($settings['custom_social_networks'] ?? '[]', true) ?: [];
                            foreach ($custom_socials as $s_item):
                            ?>
                                <div class="social-builder-row">
                                    <input type="text" class="form-control soc-name-input" value="<?= htmlspecialchars($s_item['name'] ?? '') ?>" placeholder="نام شبکه (اینستاگرام)" style="flex:1;">
                                    <input type="text" class="form-control soc-url-input" value="<?= htmlspecialchars($s_item['url'] ?? '') ?>" placeholder="آدرس لینک (https://...)" style="flex:2;">
                                    <input type="text" class="form-control soc-png-input" value="<?= htmlspecialchars($s_item['icon_png'] ?? '') ?>" placeholder="آدرس آیکون PNG/JPG (اختیاری)" style="flex:2;">
                                    <button type="button" class="btn btn-sm btn-danger" onclick="this.parentElement.remove()">✕</button>
                                </div>
                            <?php endforeach; ?>
                        </div>
                        ...
                        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:16px; margin-top:24px; border-top:1px solid var(--border); padding-top:18px;">
                            <div class="form-group">
                                <label>رنگ اصلی قالب (HEX)</label>
                                <input type="color" name="primary_color" value="<?= htmlspecialchars($settings['primary_color']) ?>" style="width:60px;height:40px;border:none;cursor:pointer;">
                            </div>
                            <div class="form-group">
                                <label>رنگ ثانویه (HEX)</label>
                                <input type="color" name="secondary_color" value="<?= htmlspecialchars($settings['secondary_color']) ?>" style="width:60px;height:40px;border:none;cursor:pointer;">
                            </div>
                        </div>
            ...
            <?php elseif ($view === 'profile'): ?>
                ...
                <div class="card" style="max-width:480px;">
                    <form method="POST" action="admin.php">
                        ...
                        <div class="form-group">
                            <label>نام کاربری مدیر</label>
                            <input type="text" name="admin_user" class="form-control" value="<?= htmlspecialchars($admin_user) ?>" required>
                        </div>
                        <div class="form-group">
                            <label>رمز عبور مدیر</label>
                            <input type="password" name="admin_pass" class="form-control" value="<?= htmlspecialchars($admin_pass) ?>" required>
                        </div>
                        <div class="form-group">
                            <label>ایمیل مدیر</label>
                            <input type="email" name="admin_email" class="form-control" value="<?= htmlspecialchars($user_row['email'] ?? 'admin@ariastore.ir') ?>">
                        </div>
    ...
    <script>
        ...
        // تغییر حالت بین ویرایشگر بصری (TinyMCE) و کد HTML خالص
        function switchEditorMode(textareaId, mode) {
            ...
            if (mode === 'visual') {
                ...
                if (editorEl) {
                    ...
                    editorEl.style.minHeight = '340px';
                }
        ...
        function insertHtml(htmlTag) {
            ...
            if (typeof tinymce !== 'undefined' && tinymce.activeEditor && !tinymce.activeEditor.isHidden()) {
                ...
                const text = el.value;
                el.value = text.substring(0, start) + htmlTag + text.substring(end);
                el.focus();
            }
        }

        // سازنده گرافیکی مگامنو (بدون نیاز به نوشتن کد JSON)
        ...
        function addMegaColumn() {
            ...
            div.className = 'mega-builder-col';
            div.innerHTML = `
                <div style="display:flex; justify-content:space-between; margin-bottom:10px;">
                    <input type="text" class="form-control col-title-input" placeholder="عنوان ستون (مثال: دیجیتال)" style="width:80%;">
                    <button type="button" class="btn btn-sm btn-danger" onclick="this.parentElement.parentElement.remove()">✕</button>
                </div>
                <div class="col-links-list">
                    <div class="mega-link-row">
        ...
        function addMegaLinkRow(btn) {
            const linksList = btn.parentElement.querySelector('.col-links-list');
            if (!linksList) return;
            const div = document.createElement('div');
        ...
        function serializeGraphicalMegaMenu() {
            ...
            const columns = [];
            document.querySelectorAll('.mega-builder-col').forEach(colEl => {
                const titleVal = colEl.querySelector('.col-title-input').value.trim();
                const links = [];
                colEl.querySelectorAll('.mega-link-row').forEach(rowEl => {
                    const lTitle = rowEl.querySelector('.link-title-input').value.trim();
                    const lUrl = rowEl.querySelector('.link-url-input').value.trim();
                    if (lTitle) links.push({ title: lTitle, url: lUrl || '?route=shop' });
                });
                if (titleVal || links.length > 0) {
                    columns.push({ title: titleVal || 'دسته', links: links });
                }
            });

            const promoTitle = document.getElementById('promo-title-input') ? document.getElementById('promo-title-input').value.trim() : '';
            const promoImg = document.getElementById('promo-image-input') ? document.getElementById('promo-image-input').value.trim() : '';
            const promoUrl = document.getElementById('promo-url-input') ? document.getElementById('promo-url-input').value.trim() : '';
            const promoObj = (promoTitle || promoImg) ? { title: promoTitle, image: promoImg, url: promoUrl || '?route=shop' } : {};

            const resultJson = JSON.stringify({ columns: columns, promo: promoObj });
            const inputEl = document.getElementById('mega-content-input');
            if (inputEl) inputEl.value = resultJson;
        }

        // سازنده شبکه‌های اجتماعی دلخواه با امکان آپلود آیکون PNG
        function addSocialNetworkRow() {
            const wrap = document.getElementById('social-networks-builder');
            if (!wrap) return;
            const div = document.createElement('div');
            div.className = 'social-builder-row';
            div.innerHTML = `
                <input type="text" class="form-control soc-name-input" placeholder="نام شبکه (توییتر)" style="flex:1;">
                <input type="text" class="form-control soc-url-input" placeholder="آدرس لینک (https://...)" style="flex:2;">
                <input type="text" class="form-control soc-png-input" placeholder="آدرس آیکون PNG/JPG" style="flex:2;">
                <button type="button" class="btn btn-sm btn-danger" onclick="this.parentElement.remove()">✕</button>
            `;
        ...
        function serializeCustomSocialNetworks() {
            ...
            document.querySelectorAll('.social-builder-row').forEach(row => {
                const nameVal = row.querySelector('.soc-name-input').value.trim();
                const urlVal = row.querySelector('.soc-url-input').value.trim();
                const pngVal = row.querySelector('.soc-png-input').value.trim();
                if (nameVal || urlVal) {
                    result.push({ name: nameVal || 'شبکه اجتماعی', url: urlVal, icon_png: pngVal });
                }
            });
            const inp = document.getElementById('custom-socials-input');
